...
exports.Config = Schema.intersect([
  Schema.object({
    KeywordPrefix: Schema.string().default('#').description('触发关键词提示前缀'),
  ...
  async function downloadImageAsBase64(imagePath) {
    try {
      if (imagePath.startsWith('http://') || imagePath.startsWith('https://')) {
        ...
        const imageBuffer = fs.readFileSync((process.platform == 'win32' ? '' : '/') + imagePath);
      ...
      if (fs.existsSync(filePath)) {
        ...
        for (const keyword in data) {
          ...
          if (keyword.startsWith('regex:')) {
            ...
            isMatch = anothercontent === config.KeywordPrefix + keyword;
          }
          if (isMatch) {
            const now = Date.now();
